...
handler.all = async function (m, { isPrems }) {
    ...
    if (/^.*tiktok/i.test(m.text)) {
        let res = await fetch(API('lolhum', '/api/tiktok', 'apikey', { url }))
        if (!res.ok) return m.reply(eror)
        let json = await res.json()
        await m.reply(wait)
        let caption = `Username: ${json.result.author.username}\nCaption: ${json.result.description}`
        // m.reply(util.format(json))
        await this.sendFile(m.chat, json.result.link, '', caption, m)
    }
